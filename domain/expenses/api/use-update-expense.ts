...
import type { UpdateExpenseRequest, ExpenseResponse } from "./types"
...
async function updateExpense(expenseId: string, data: UpdateExpenseRequest): Promise<ExpenseResponse> {
  const raw = await apiRequest<Record<string, unknown>>(`/api/expenses/${encodeURIComponent(expenseId)}`, {
    method: "PUT",
